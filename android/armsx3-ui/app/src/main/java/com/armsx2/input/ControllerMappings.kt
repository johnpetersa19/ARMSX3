...
object ControllerMappings {
    ...
    const val P1 = 0
    const val P2 = 1
    // Mapping tier by unified pad slot. Every one of the PS3's seven ports gets its
    // own remap tier, because they are all real ports -- unlike the PS2, where slots
    // 2-7 only existed behind a multitap accessory and therefore shared P1's config.
    //
    // The key shape is unchanged for the two players that already had one: P1 keeps
    // the bare keys and P2 keeps "p2.", so existing mappings survive the upgrade.
    // This is the single chokepoint every per-player lookup (targetForPhysical, stick
    // modes, turbo, custom-stick) routes through.
    ...
    fun setHapticIntensity(pct: Int) {
        ...
        com.armsx3.NativeApp.sHapticScale = clamped / 100f
    }
    /** Push the persisted haptic strength into the native gate; call once at app start. */
    fun syncHapticIntensity() {
        com.armsx3.NativeApp.sHapticScale = hapticIntensity() / 100f
    }

    // Multitap is gone: it was a PS2 accessory that split two physical ports into
    // four slots each. The PS3 exposes seven ports natively (CELL_PAD_MAX_PORT_NUM),
    // so there is no switch to offer -- PadRouter always fills all seven.
    ...
    enum class SysHotkey(val prefKey: String, val label: String) {
        ...
        // TEXTURE_DUMP removed: dumping GS textures by hash is a PCSX2 feature.
